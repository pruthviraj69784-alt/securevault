...
class QRSessionService {
    async createQRSession(userId, fileId, options = {}) {
        ...
        const reqUserId = String(userId && (userId.id || userId._id || userId));
        ...
        const version = (file.versions || []).find(v => v.version === targetVersionNum) || (file.versions && file.versions[0]);
        ...
        if (version.status === "PROCESSING") {
            throw new AppError("File is still undergoing security scanning and sensitive data masking. Please wait a moment until processing completes before generating a QR code.", 409);
        }
        ...
        const sessionData = {
            ...
            maxDownloads: 1,
            downloadCount: 0,
            autoMask: options.autoMask !== false
        };

        await qrSessionRepository.createSession(sessionData, ttlSeconds);

        if (options.autoMask !== false && (file.hasSensitiveData || options.isRedacted)) {
    ...
    async consumeQRSession(sessionId, nonce, userId = null, clientIp = "127.0.0.1") {
        ...
        // Automatically mask sensitive data for recipient downloading via QR if session.autoMask is enabled
        const shouldMask = session.autoMask !== false && (file.hasSensitiveData || file.isRedacted);
        if (shouldMask) {
        ...
        await auditService.logAction({
            ...
            ip: clientIp,
            resourceId: file._id,
            details: { sessionId, filename: file.originalName, masked: shouldMask }
        });
